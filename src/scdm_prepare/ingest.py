...
def _normalize_columns(df: pl.DataFrame, table_name: str) -> pl.DataFrame:
    """Rename source columns to match schema casing and add missing columns as NULL.

    SAS source files use inconsistent casing (e.g. ``patid`` vs ``PatID``).
    This performs a case-insensitive rename to the canonical schema names,
    then adds any columns present in the schema but absent from the source
    as null ``Utf8`` columns.
    """
    schema_cols = TABLES[table_name].columns
    lower_to_schema = {c.lower(): c for c in schema_cols}

    rename_map = {}
    for src_col in df.columns:
        if src_col == "samplenum":
            continue
        schema_name = lower_to_schema.get(src_col.lower())
        if schema_name and schema_name != src_col:
            rename_map[src_col] = schema_name
...
def ingest_table(
    ...
    for samplenum in subsamples:
        ...
        if file_ext == ".parquet":
            ...
            chunks = []
            try:
                for chunk_df, _meta in pyreadstat.read_file_in_chunks(
